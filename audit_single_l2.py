...
def parse_config(text: str) -> dict:
    ...
    for m in re.finditer(r"(?m)^interface\s+(\S+)\s*", text):
        ...
        cfg["interfaces"][name] = {
            ...
            "zone": zone,
            "is_tunnel": is_tunnel,
            # L2/SW fields (optional for switches)
            "sw_mode":   (re.search(r"^\s*switchport mode\s+(access|trunk)\b", body_clean, re.M).group(1) if re.search(r"^\s*switchport mode\s+(access|trunk)\b", body_clean, re.M) else None),
            "access_vlan": (int(re.search(r"^\s*switchport access vlan\s+(\d+)", body_clean, re.M).group(1)) if re.search(r"^\s*switchport access vlan\s+(\d+)", body_clean, re.M) else None),
            "trunk_allowed": (re.search(r"^\s*switchport trunk allowed vlan\s+(.+)$", body_clean, re.M).group(1).strip() if re.search(r"^\s*switchport trunk allowed vlan\s+(.+)$", body_clean, re.M) else None),
            "trunk_native":  (int(re.search(r"^\s*switchport trunk native vlan\s+(\d+)", body_clean, re.M).group(1)) if re.search(r"^\s*switchport trunk native vlan\s+(\d+)", body_clean, re.M) else None),
            "bpduguard": bool(re.search(r"^\s*spanning-tree\s+bpduguard\s+enable\b", body_clean, re.M)),
            "portfast":  bool(re.search(r"^\s*spanning-tree\s+portfast\b", body_clean, re.M)),
            "storm": {
                "broadcast": (float(re.search(r"^\s*storm-control\s+broadcast\s+level\s+([\d\.]+)", body_clean, re.M).group(1)) if re.search(r"^\s*storm-control\s+broadcast\s+level\s+([\d\.]+)", body_clean, re.M) else None),
                "multicast": (float(re.search(r"^\s*storm-control\s+multicast\s+level\s+([\d\.]+)", body_clean, re.M).group(1)) if re.search(r"^\s*storm-control\s+multicast\s+level\s+([\d\.]+)", body_clean, re.M) else None),
                "unicast":   (float(re.search(r"^\s*storm-control\s+unicast\s+level\s+([\d\.]+)", body_clean, re.M).group(1)) if re.search(r"^\s*storm-control\s+unicast\s+level\s+([\d\.]+)", body_clean, re.M) else None),
            },
            "dhcp_snooping_trust": bool(re.search(r"^\s*ip dhcp snooping trust\b", body_clean, re.M)),
            "arp_inspection_trust": bool(re.search(r"^\s*ip arp inspection trust\b", body_clean, re.M)),
            "shutdown": bool(re.search(r"^\s*shutdown\b", body_clean, re.M)),
            "cdp_disabled": bool(re.search(r"^\s*no cdp enable\b", body_clean, re.M)),
            # Tunnel details (if any)
            "tunnel_mode": (re.search(r"^\s*tunnel mode\s+(\S+)", body_clean, re.M).group(1) if re.search(r"^\s*tunnel mode\s+(\S+)", body_clean, re.M) else None),
            "tunnel_source": (re.search(r"^\s*tunnel source\s+(\S+)", body_clean, re.M).group(1) if re.search(r"^\s*tunnel source\s+(\S+)", body_clean, re.M) else None),
            "tunnel_destination": (re.search(r"^\s*tunnel destination\s+(\S+)", body_clean, re.M).group(1) if re.search(r"^\s*tunnel destination\s+(\S+)", body_clean, re.M) else None),
    ...
    # ---- Global L2 features (optional) ----
    cfg.setdefault("global", {})
    cfg["global"]["dhcp_snooping"] = bool(re.search(r"^ip dhcp snooping\b", text, re.M))
    cfg["global"]["dhcp_snooping_vlans"] = re.findall(r"^ip dhcp snooping vlan\s+(.+)$", text, re.M)
    cfg["global"]["arp_inspection_vlans"] = re.findall(r"^ip arp inspection vlan\s+(.+)$", text, re.M)
    _vtp = re.search(r"^vtp mode\s+(\S+)", text, re.M) or re.search(r"^vtp\s+(\S+)", text, re.M)
    cfg["global"]["vtp_mode"] = (_vtp.group(1) if _vtp else None)
    _stp = re.search(r"^spanning-tree mode\s+(\S+)", text, re.M) or re.search(r"^spanning-tree mode\s+(\S+-\S+)", text, re.M)
    cfg["global"]["stp_mode"] = (_stp.group(1) if _stp else None)
    cfg["global"]["http_server"]  = bool(re.search(r"^ip http server\b", text, re.M))
    cfg["global"]["https_server"] = bool(re.search(r"^ip http secure-server\b", text, re.M))
    cfg["global"]["ssh_v2"] = bool(re.search(r"^ip ssh version\s+2\b", text, re.M))
    cfg["global"]["aaa"]    = bool(re.search(r"^aaa new-model\b", text, re.M))
    cfg["global"]["enable_secret"] = bool(re.search(r"^enable secret\b", text, re.M))
    cfg["global"]["snmp_v3"] = bool(re.search(r"^snmp-server group\s+\S+\s+v3\b", text, re.M))
...
def run_checks(cfg):
    ...
    for ifname, idef in cfg.get("interfaces", {}).items():
        # Если нет IP - пропускаем
        if not idef.get("ip"):
            continue

        # HIGH: внешний outside без входящего ACL
        ...
        # Tunnel/Loopback — ACL обычно не требуется (OK)
        if idef.get("is_tunnel") or ifname.lower().startswith(("loopback","vti","dialer")):
            findings.append(_finding("ok","iface_tunnel_no_acl_ok",f"interface {ifname}",
                "Tunnel/Loopback без ACL — допустимо. Фильтрация на outside или ZBF.", cfg))
            continue
    ...
    # ДОБАВЛЕНО: L2-проверки для коммутаторов
    findings.extend(run_switch_checks(cfg))
...
def run_switch_checks(cfg):
    f = []
    # Heuristic: treat as switch if any interface has sw_mode
    is_switch = any((idef.get("sw_mode") for idef in cfg.get("interfaces", {}).values()))
    if not is_switch:
        return f
    ...
    if g and not g.get("aaa"):
        f.append(_finding("medium","aaa_absent","global",
            "AAA не включён (aaa new-model).", cfg,
            fix="Включить AAA, login по локальным/Radius учеткам, VTY только SSH."))

    if g and not g.get("ssh_v2"):
        f.append(_finding("medium","ssh_v1_or_absent","global",
            "SSH v2 не принудительно (ip ssh version 2).", cfg,
            fix="Добавить 'ip ssh version 2'. VTY: 'transport input ssh'."))

    if g and g.get("http_server"):
        f.append(_finding("medium","http_server_enabled","global",
            "Включён нешифрованный HTTP на устройстве.", cfg,
            fix="Выполнить 'no ip http server'. Если нужен GUI — 'ip http secure-server' + ACL."))

    if not g.get("enable_secret"):
        f.append(_finding("medium","no_enable_secret","global",
            "Отсутствует 'enable secret' (хешированный).", cfg,
            fix="Задать 'enable secret <strong>' и удалить 'enable password' если есть."))

    if not g.get("dhcp_snooping"):
        f.append(_finding("medium","dhcp_snooping_off","global",
            "DHCP Snooping не включён.", cfg,
            fix="Включить 'ip dhcp snooping' и указать VLAN'ы: 'ip dhcp snooping vlan <list>'."))
    elif g.get("dhcp_snooping") and not g.get("arp_inspection_vlans"):
        f.append(_finding("low","dai_not_configured","global",
            "Dynamic ARP Inspection не включён.", cfg,
            fix="Добавить 'ip arp inspection vlan <list>' и trust на аплинках."))

    if g.get("vtp_mode") and str(g["vtp_mode"]).lower() in ("server","client"):
        f.append(_finding("low","vtp_non_transparent","global",
